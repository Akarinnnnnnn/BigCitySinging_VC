# 借用 https://github.com/prophesier/diff-svc/blob/main/infer_tools/slicer.py
...
if __name__ == "__main__":
    ...
    # wav, sr = sf.read('极乐净土（新）_0_乐正绫.wav')
    # slicer = Slicer(sr=sr)
    # chunk_dict = slicer.slice(wav)
    # for key in chunk_dict:
    #     print(chunk_dict[key])
    filename = 'Nostalgia_0_乐正绫.wav'
    chunks = cut(filename)
    result, sr = chunks2audio(filename, chunks=chunks)
    for i,item in enumerate(result):
        if item[0] != True:
            sf.write(filename.replace('.wav', '_'+str(i)+'.wav'), item[1], samplerate=sr)